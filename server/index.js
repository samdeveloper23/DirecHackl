...
//utilizamos la variable de app previamente cargada con express para la creación de rutas.
app.get('/hi-hack-a-boss', (req, res) => {

    //cuando nos hagan una peticion get les devolvemos (res = respuesta) un mensaje.
    res.status(200).send('Hola a todos los participantes de este taller inpartido en HACK A BOSS');
...
io.on('connection', (socket) => {  /*llamamos a socket para obtener las ropiedades de sus metodos*/
    ...
    socket.on('add-message', (data) => {
        ...
        //si deseamos ver el ultimo mensaje enviado podemos declarar una variable y usando 'length' podemos hacer 'log' del ultimo mensaje emitido. 
